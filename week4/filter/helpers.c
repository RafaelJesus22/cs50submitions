...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            // all the three properties are changed to gray
...
// ensure the color value of each pixel is a integer between 0 and 255
int ensureIntColorValue(int RGB)
{
    if (RGB > 255)
    {
        RGB = 255;
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            sepiaBlue = ensureIntColorValue(round(0.272 * image[j][i].rgbtRed + 0.534 * image[j][i].rgbtGreen + 0.131 * image[j][i].rgbtBlue));
            sepiaGreen = ensureIntColorValue(round(0.349 * image[j][i].rgbtRed + 0.686 * image[j][i].rgbtGreen + 0.168 * image[j][i].rgbtBlue));
            sepiaRed = ensureIntColorValue(round(0.393 * image[j][i].rgbtRed + 0.769 * image[j][i].rgbtGreen + 0.189 * image[j][i].rgbtBlue));
